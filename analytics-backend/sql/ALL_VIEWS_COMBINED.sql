...
adjusted_metrics AS (
  ...
  SELECT
    ...
    exit_count,
    -- For unique sessions: if had_exit but not had_view, assume had_view
...
scored AS (
  SELECT
    *,
    -- Section health score (cap exit_rate at 100 for legacy data)
    (
      (avg_engagement_rate * 2) +  -- High engagement is good
      (avg_scroll_depth_percent * 0.5) +   -- Scroll depth matters
      (100 - LEAST(100.0, avg_exit_rate)) * 0.3  -- Low exit rate is good
    ) AS health_score
...
SELECT
  ...
  total_unique_exits,
  total_unique_viewers,
  -- Cap at 100% to handle legacy data with asymmetric tracking bug
  ROUND(LEAST(100.0, avg_exit_rate), 2) AS avg_exit_rate,
